 ...
 * File: Program.cs
 * Description of file: This program reads in 3 input files (people, house, appartment) and creates a Community Object. 
                             Community can then be altered to add, remove properties and people. It can also be used to
                             purchase or mark for sale of properties in the Community.
 * */
...
namespace myGroupName_Assign1
{
    class Program
    {
        ...
        {
            // create the community from file
            string[] arglist = new string[3];
            arglist[0] = "99999";
            arglist[1] = "Dekalb";
            arglist[2] = "0";
            Community community1 = new Community(arglist);

            // Variables used with assisting file input and Object Construction
            String[] propIn = new String[8];
            string fileInString;

            // read in the people from file
            using (StreamReader inFile = new StreamReader("../../../Persons_Input.txt"))
            {
                fileInString = inFile.ReadLine();
                while (fileInString != null)
                {
                    propIn = fileInString.Split('\t');
                    community1.residents.Add(new Person(propIn));
                    fileInString = inFile.ReadLine();
                }
                // close file
                inFile.Close();
            ...
            // read in houses from file
            using (StreamReader inFile = new StreamReader("../../../house_input.txt"))
            {
                fileInString = inFile.ReadLine();
                while (fileInString != null)
                {
                    propIn = fileInString.Split('\t');
                    community1.props.Add(new House(propIn));
                    fileInString = inFile.ReadLine();
                }
                // close file
                inFile.Close();
            ...
            // read in appartments
            using (StreamReader inFile = new StreamReader("../../../apartment_input.txt"))
            {
                fileInString = inFile.ReadLine();
                while (fileInString != null)
                {
                    propIn = fileInString.Split('\t');
                    community1.props.Add(new Apartment(propIn));
                    fileInString = inFile.ReadLine();
                }
                // close file
                inFile.Close();
